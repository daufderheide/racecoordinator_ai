...
public class OverallStandingsTest {
    ...
    private Heat createHeat(int number, RaceParticipant p1, float laps1, float time1, RaceParticipant p2, float laps2, float time2) {
        List<DriverHeatData> dhdList = new ArrayList<>();
        
        DriverHeatData d1 = new DriverHeatData(p1);
        // Add laps to simulate average time
        for(int i=0; i<laps1; i++) d1.addLap(time1/laps1); 
        
        DriverHeatData d2 = new DriverHeatData(p2);
        for(int i=0; i<laps2; i++) d2.addLap(time2/laps2);

        dhdList.add(d1);
        dhdList.add(d2);
        
        RaceScoring scoring = new RaceScoring(FinishMethod.Lap, 10, HeatRanking.LAP_COUNT, HeatRankingTiebreaker.FASTEST_LAP_TIME);
