...
test.describe("Input Dialog Visuals", () => {
  ...
  test("should display input dialog for adding LED string", async ({
    page,
  }) => {
    ...
    const ledSectionHeader = arduinoEditor.locator(
      ".led-config-section .section-header",
    );
    await ledSectionHeader.waitFor({ state: "visible" });
    await ledSectionHeader.click();

    // Click Add LED String button
    const addLedStringBtn = ledSectionHeader.locator(
