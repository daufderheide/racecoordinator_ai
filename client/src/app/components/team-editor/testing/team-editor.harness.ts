...
export class TeamEditorHarness extends ComponentHarness implements TeamEditorHarnessBase {
  static hostSelector = 'app-team-editor';

  protected getNameInput = this.locatorFor('input.dm-input');
  protected getSaveBtn = this.locatorFor('.btn-save');
  protected getSaveAsNewBtn = this.locatorFor('.btn-save-new');
  protected getDriverItems = this.locatorForAll('.driver-item');
  protected getAvatarSelector = this.locatorFor('app-image-selector .image-preview');
