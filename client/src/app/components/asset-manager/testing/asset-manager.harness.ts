...
export class AssetManagerHarness extends ComponentHarness implements AssetManagerHarnessBase {
  static hostSelector = 'app-asset-manager';

  protected getDbName = this.locatorFor('.active-db-name');
  protected getTotalSizeText = this.locatorFor('.usage-text');
  protected getAssetCards = this.locatorForAll('.asset-card');
  protected getFilterTabs = this.locatorForAll('.filter-tabs .tab');
  protected getBackButton = this.locatorFor('app-back-button button');
  ...
  async getAssetCardName(index: number): Promise<string> {
    ...
    if (index < cards.length) {
      const nameEl = await cards[index].locatorFor('.asset-name')();
