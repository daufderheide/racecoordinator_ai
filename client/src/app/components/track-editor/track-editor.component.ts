...
export class TrackEditorComponent implements OnInit, OnDestroy, DirtyComponent {
  ...
  toggleAllSections(forcedState?: boolean) {
    ...
    if (next) {
      ...
      this.cameraEditors?.forEach((e) => e.ensureSectionsExpanded());
    }
  }
  ...
  private syncInterfaceConfigsFromTrack(track: Track) {
    ...
    if (track.camera_configs && track.camera_configs.length > 0) {
      const newCamConfigsJson = JSON.stringify(track.camera_configs);
      if (newCamConfigsJson !== JSON.stringify(this.cameraConfigs)) {
        this.cameraConfigs = JSON.parse(newCamConfigsJson);
      }
    } else if (this.cameraConfigs.length > 0) {
      this.cameraConfigs = [];
    }
  }
  ...
  private ensureSectionsExpandedForHelp() {
    ...
    if (this.cameraConfigs?.length > 0 && this.cameraEditors?.first) {
      this.cameraEditors.first.ensureSectionsExpanded();
    }
